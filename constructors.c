...
int main()
{
    ...
    while ((m = 5))
    {
        ...
        else if (answer1a == 2)
        {
            ...
            printf("Insert into constructorRef?\n>>> ");
            ...
            char added_constructors[maximum_array];
            memset(added_constructors, '\0', sizeof(added_constructors));

            sprintf(added_constructors, "INSERT into %s(constructorRef, name, nationality, url)VALUES(\'%s\', \'%s\', \'%s\', \'%s\');",
                    table_name, constructorRef, name, nationality, url);

            mysql_query(con, added_constructors);
            sleep(3);
        }
