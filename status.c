...
int main()
{
    ...
    while ((m = 5))
    {
        ...
        else if (answer1a == 2)
        {
            ...
            char added_status[maximum_array];
            memset(added_status, '\0', sizeof(added_status));

            sprintf(added_status, "INSERT into %s(status)VALUES(\'%s\');",
                    table_name, status);
#ifdef DEBUG
    printf("\nyour query is %s\n", added_status);
#endif                   

            mysql_query(con, added_status);
            sleep(3);
        }
