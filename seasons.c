...
int main()
{
    ...
    while ((m = 5))
    {
        ...
        else if (answer1a == 2)
        {
            ...
            char added_seasons[maximum_array];
            memset(added_seasons, '\0', sizeof(added_seasons));

            sprintf(added_seasons, "INSERT into %s(year, url)VALUES(\'%d\', \'%s\');",
                    table_name, year, url);
#ifdef DEBUG
    printf("\nyour query is %s\n", added_seasons);
#endif                   

            mysql_query(con, added_seasons);
            sleep(3);
        }
